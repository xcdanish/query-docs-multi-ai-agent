...
from app.ai.prompts.supervisor import SYSTEM_PROMPT
...
def supervisor_node(state: AgentState) -> dict:
    ...
    for msg in last_messages:
        if isinstance(msg.content, list):
            text_only = " ".join([c["text"]
                                 for c in msg.content if c["type"] == "text"])
    ...
    # Safety Fallback: Ensure vision is only routed if an image is actually present
    if next_agent == "vision" and not has_image:
        next_agent = "research"
